...
const PDF_RENDER_API = ((import.meta.env.VITE_PDF_RENDER_API as string | undefined) ?? '')
  .trim()
  .replace(/\/$/, '');

export function isPdfRenderBackendConfigured(): boolean {
  return Boolean(PDF_RENDER_API);
}
...
export async function cropDocumentFiguresWithBackend(
  ...
  if (!PDF_RENDER_API || figures.length === 0) return result;
 ...
 * 1. Backend PyMuPDF nếu đã cấu hình VITE_PDF_RENDER_API.
...
export async function renderPdfToImages(
  ...
  try {
    ...
    throw new Error(
      `${browserError} Chưa cấu hình VITE_PDF_RENDER_API, vì vậy ứng dụng không có ` +
