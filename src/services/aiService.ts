...
import { cropDocumentFiguresWithBackend, cropImageDataUrlPrecise } from './presentationPdfUtils';
...
export async function processOriginalDocument(
  ...
  // Gemini 3 được khuyến nghị dùng nhiệt độ mặc định. Không ép temperature=0.1.
  // Dùng mức output tối đa của các model Gemini/Gemma đang chọn để tránh
  // tài liệu nhiều trang bị dừng giữa chừng.
  const config = {
  ...
  try {
    ...
    if (!fullText.trim()) {
      // Một số phiên bản SDK/model không phát text qua stream. Thử lại một lần bằng generateContent.
    ...
    throw new Error(toReadableApiError(error, options.model));
  }
}
...
export async function refineDocumentImageMarkers(
  ...
  const apiKey = options.apiKey.trim();
  if (!apiKey) return options.markdown;
